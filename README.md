...
A high-precision, 24/7 autonomous Telegram bot designed for tracking multiple satellites simultaneously. Built with Python and the **Skyfield/SGP4** propagation models, it acts as a personal fleet command center, providing real-time orbital event notifications and automated 7-day pass forecasts with scientific accuracy.

## ✨ Key Features

* **Multi-Satellite Fleet Tracking:** Monitor an unlimited number of satellites at the same time. Adding new satellites seamlessly integrates them into your fleet without overwriting your existing alarms.
* **Automated 7-Day Scheduling:** Instantly receive a fully calculated 7-day pass calendar whenever you add a satellite, change your ground station, or during daily morning maintenance.
...
* **Smart Elevation Filter:** Avoid "junk" passes. Use `/minelevation` to only receive alerts and schedule reports for high-quality passes above your chosen horizon threshold.
...
* **Auto-Maintenance:** Daily TLE refreshes at 06:00 AM (Europe/Istanbul) to maintain sub-second precision, prevent orbital drift, and deliver your daily pass schedules.
...
* `/satellite <NORAD_ID1> <ID2> ...` - Adds one or more satellites to your active fleet (e.g., `/satellite 39030 25544`).
* `/constellation` - Instantly loads the Turkish national fleet (GÖKTÜRK-2, İMECE, GÖKTÜRK-1A) to your tracking list.
...
* `/tleupdate` - Manually forces a refresh of orbital elements from CelesTrak and regenerates 7-day schedules.
